...
Function CheckIn-AllCheckedOutFiles() { 
    ...
    Param(
        [Parameter(Mandatory = $true)] [string]$SiteColUrl,
        [Parameter(Mandatory = $true)] [string]$ReportOutput
    )
  
    #Get the Web Application
    #  $WebApp = Get-SPWebApplication $WebAppURL
    # $WebApp = Get-SPWebApplication $SiteColUrl.rootweb
    ...
    $reqSiteCol = Get-SPSite $SiteColUrl

    #Loop through each site in the site collection
    ForEach ($Web in $reqSiteCol.AllWebs) {
            ...
            if ( ($List.Hidden -eq $false) -and ($SystemLists -notcontains $List.Title) ) {
                ...
                foreach ($ListItem in $List.Items) {
                    If ( ($ListItem.File.CheckOutStatus -ne "None") -and ($ListItem.File.CheckedOutByUser -ne $null)) {
                        ...
                        #Check in the file
                        $ListItem.File.Checkin("Checked-In by Sharepoint Administrator as part of Migration effort.")
                        write-host -f Green "File Checked-In Successfully!"
                    }
                }
            }
        }
...
#CheckIn-AllCheckedOutFiles -WebAppURL -ReportOutput "C:\PSExports\ISCheckedOutFiles.CSV"
CheckIn-AllCheckedOutFiles
